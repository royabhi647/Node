...
A board that redirects the ball to the right spans 
the top-left corner to the bottom-right corner and is represented in the grid as 1
...
We drop one ball at the top of each column of the box. Each ball can get stuck in the box or fall
 out of the bottom. A ball gets stuck if it hits a "V" shaped pattern between two boards or
...
class Solution {
    public int[] findBall(int[][] grid) {
        ...
        int m = grid[0].length;
        int ans[] = new int[m];
        // ball fall from 1st col start
        for(int j=0;j<m;j++){
            int cpos = j; // current position
